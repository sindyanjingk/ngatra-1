...
export default function ModalAddDiscount({ users }: { users: TUsers }) {
    const modal = useModal();

    return (
        <form
            action={async (data: FormData) => {
                const response = await addDiscountAction(data, users)   
                if(response.status) {
        ...
        >
            ...
            <div className="flex flex-col items-center justify-center space-y-4 p-6 md:p-8">
                ...
                <div className="flex flex-col gap-y-2 w-full">
                    ...
                    <Input id="discount" type="number" name="discount" placeholder="Total discount" />
                </div>
            </div>
