...
import { banUsersAction, createCategory } from "../../lib/action";
import { TUsers } from "@/app/app/(dashboard)/site/[id]/users/page";



export default function ModalBanAccount({ users }: { users: TUsers[] }) {
    const modal = useModal();

    return (
        <form
            action={async (data: FormData) => {
                const response = await banUsersAction(data, users)
                if(response.status) {
        ...
        >
            ...
            <div className="flex flex-col items-center justify-center space-y-4 p-6 md:p-8">
                ...
                <div className="flex flex-col gap-y-2 w-full">
                    <label htmlFor="category">Are you sure want to ban users?</label>
                </div>
            </div>
