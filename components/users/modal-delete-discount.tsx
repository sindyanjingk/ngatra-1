...
import { deleteDiscountAction } from "../../lib/action";
import { TUsers } from "@/app/app/(dashboard)/site/[id]/users/page";



export default function ModalDeleteDiscount({ users }: { users: TUsers[] }) {
    const modal = useModal();
    
    return (
        <form
            action={async (data: FormData) => {
                const response = await deleteDiscountAction(data, users)   
                if(response.status) {
        ...
        >
            ...
            <div className="flex flex-col items-center justify-center space-y-4 p-6 md:p-8">
                ...
                <div className="flex flex-col gap-y-2 w-full">
                    <label htmlFor="discount">{`Are you sure want to delete discount for this users?`}</label>
...
function CreateCategoryButton() {
    ...
    return (
        <button
            ...
            disabled={pending}
        >
            {pending ? <Loader2Icon className="animate-spin" size={18} /> : "Delete"}
        </button>
    );
}
