...
import { deleteAllServices } from "@/lib/service-action";
import { toast } from "sonner";


export default function ModalDeleteAllServices({selectedServices}: {selectedServices: string[]}) {
    const modal = useModal();
    return (
        <form
            action={async (data: FormData) => {
                const response = await deleteAllServices(selectedServices)
                if(response.success){
        ...
        >
            <div className="flex items-center justify-center p-8">
                <div className="text-lg font-bold">Are you sure want to delete selected services?</div>
            </div>
...
function CreateSerciceFormButton() {
    ...
    return (
        <Button
            ...
            disabled={pending}
        >
            {pending ? <Loader2Icon className="animate-spin" /> : "Delete All"}
        </Button>
    );
}
