...
import { enableAllServices } from "@/lib/service-action";
import { toast } from "sonner";


export default function ModalEnableAll({selectedServices}: {selectedServices: string[]}) {
    const modal = useModal();
    return (
        <form
            action={async (data: FormData) => {
                const response = await enableAllServices(selectedServices)
                if(response.success){
        ...
        >
            <div className="flex items-center justify-center p-8">
                <div className="text-lg font-bold">Are you sure want to enable selected services?</div>
            </div>
...
function CreateSerciceFormButton() {
    ...
    return (
        <Button
            ...
            disabled={pending}
        >
            {pending ? <Loader2Icon className="animate-spin" /> : "Enable All"}
        </Button>
    );
}
