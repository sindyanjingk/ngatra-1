...
export default function ImportServicesModal({ siteId, providers, categories }: { siteId: string, providers: siteProviders[], categories: category[] }) {
    ...
    const [isLoadingCrate, setIsloadingCreate] = useState(false)
    ...
    const isAllSelected = selected.length === services.length;
    const isSomeSelected = selected.length > 0 && !isAllSelected;

    const toggleSelectAll = () => {
        setSelected(isAllSelected ? [] : [...services]);
    };
    ...
    const getServices = async () => {
        setIsloading(true)
        try {
            const response = await axios.post(`/api/services`, { siteId, url: selectedProvider.url, name: selectedProvider.name, apiKey: selectedProvider.apiKey })
    ...
    return (
        ...
        >
            {/* Form Header */}
            {
                isNext ?
                    <div className="flex flex-col items-center justify-center space-y-4 p-6 md:p-8">
                        ...
                        <div className="my-4 border-b pb-2 flex items-start justify-between w-full">
                            <div className="text-md font-semibold">Services</div>
                            <div className="text-md font-semibold">Provider Price</div>
                            <div className="text-md font-semibold">Your Price</div>
                            <ArrowRight />
                            <div className="text-md font-semibold">Extra Price %</div>
                        </div>
            ...
            {/* Form Actions */}
            {
                isNext ?
                    <div className="flex items-center justify-end space-x-2 rounded-b-lg border-t border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
                        <Button onClick={createServices} type="button">{isLoadingCrate ? <Loader2Icon className="animate-spin" size={18} /> : `Add ${selected.length} services`}</Button>
                    </div>
                    :
                    <div className="flex items-center justify-end space-x-2 rounded-b-lg border-t border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
                        <button
